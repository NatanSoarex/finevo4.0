...
// Gerenciamento persistente do Bot do Telegram
...
function getTelegramConfig() {
  try {
    ...
    console.warn("[Telegram Config] Arquivo não encontrado, criando padrão.");
  }
  return {
    token: "8989705478:AAFSQgfEOlp4RSFmvF8fCRZi5rwbgB5GPpE",
    buttonText: "Jogar FinEvo"
  };
}

function saveTelegramConfig(token: string, buttonText: string) {
  try {
    fs.writeFileSync(TELEGRAM_CONFIG_FILE, JSON.stringify({ token, buttonText }, null, 2), "utf-8");
    console.log("[Telegram Config] Configurações armazenadas em disco.");
  } catch (e: any) {
    console.error("[Telegram Config] Falha ao gravar configurações:", e.message);
  }
}

// Rota do Webhook de entrada do Telegram Bot para responder ao comando /start
app.post("/api/telegram/webhook", async (req, res) => {
  const { update_id, message } = req.body;
  
  if (!message) {
    // Retorna sempre status OK para o Telegram saber que recebemos
  ...
  if (chatId) {
    ...
    // Se o usuário iniciar a conversa via /start, responder imediatamente com o botão do Mini App
    if (text.startsWith("/start") || text.toLowerCase().includes("jogar") || text.toLowerCase().includes("play")) {
      ...
      try {
        ...
        await fetch(tgSendMessageUrl, {
          ...
          body: JSON.stringify({
            chat_id: chatId,
            text: `Olá, *${message.from?.first_name || "Investidor"}*! 🚀\n\nSeja muito bem vindo ao *FinEvo - Simulador de Investimentos*!\n\nEste é o maior simulador financeiro gamificado dentro do Telegram! Gerencie seu próprio escritório financeiro, compre ações da B3 reais, acumule XP e suba no ranking mudial com seus amigos!\n\n👇 Toque no botão azul abaixo para abrir a sua mesa de operações agora mesmo:`,
            parse_mode: "Markdown",
            reply_markup: {
              inline_keyboard: [
                [
                  {
                    text: `🎮 ${config.buttonText || "Jogar FinEvo"}`,
                    web_app: {
                      url: appUrl
                    }
                  }
        ...
        console.log(`[Telegram Webhook Response] Mensagem de boas-vindas enviada para chatId: ${chatId}`);
      } catch (err: any) {
        console.error("[Telegram Webhook Error] Falha ao responder via sendMessage:", err.message);
      }
    }
  }
...
// Rota de configuração do Mini App do Telegram com o BotFather
app.post("/api/telegram/setup", async (req, res) => {
  const { token, buttonText } = req.body;
  if (!token) {
    return res.status(400).json({ ok: false, error: "O Token do bot é obrigatório." });
  }
  const cleanToken = token.trim();
  const cleanButtonText = (buttonText || "Jogar FinEvo").trim().substring(0, 12);
  ...
  try {
    ...
    const tgUrl = `https://api.telegram.org/bot${cleanToken}/setChatMenuButton`;
    ...
    if (response.ok && data.ok) {
      // 2. Salva as configurações em arquivo local
      saveTelegramConfig(cleanToken, cleanButtonText);

      // 3. Cadastra o Webhook oficial do Bot apontando para a nossa rota do servidor do Cloud Run
      const webhookUrl = `${appUrl}/api/telegram/webhook`;
      const tgWebhookUrl = `https://api.telegram.org/bot${cleanToken}/setWebhook?url=${encodeURIComponent(webhookUrl)}`;
      const webhookResponse = await fetch(tgWebhookUrl);
      const webhookData = await webhookResponse.json();
      
      console.log(`[Telegram Manual Setup] Webhook registrado com sucesso:`, webhookData);

      return res.json({ 
        ok: true, 
        message: "Bot configurado com sucesso! O menu persistente e o Webhook do de comando /start estão online.",
        appUrl
      });
    } else {
      return res.status(400).json({ 
        ok: false, 
        error: data.description || "Erro retornado pelo Telegram. Verifique se o Token do bot está correto." 
      });
    }
  } catch (err: any) {
    return res.status(500).json({ ok: false, error: "Erro de conexão com a API do Telegram: " + err.message });
  }
});
